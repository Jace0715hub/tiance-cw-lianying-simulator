...
双影子实现使用`damage`和`value`两种显式谱系类型，分别统计边界名次、最终核心候选与伤害；相同机制状态只占一个槽，避免把重复状态伪装为额外预算。180秒screen中，“纯伤害1+价值1”与“纯伤害2”的最佳影子核心都低基础7,899,406.90，且两组基础粗排前6名完全相同，没有影子进入最终突重排短名单。混合组价值核心低8,908,417.37；对照组第二纯伤害核心低8,868,712.08，后者高39,705.28。混合组执行39,560/29,955次展开/合法转移，对照为39,132/29,612，额外模型成本没有带来收益。至此一段、两段、自适应尾部、权重/名次消融以及等预算双影子均完成；线性价值模型保留为离线诊断，不再扩展为默认在线剪枝策略。

在线输出的每个`segments[].valueShadowDiagnostics`记录影子状态键、谱系、即时伤害名次、模型预测、混合分数和关键资源摘要，可用于判断价值收益是否偿还了即时伤害债务；诊断不参与候选选择或最终结算。核心摘要另按纯伤害与模型价值类型分别记录候选数、最佳伤害和相对基础核心差值。
