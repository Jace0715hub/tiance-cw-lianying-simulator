...
- 正式技能轴：[`output/lianying-free-fixed-180s-pruned-suffix-revival.json`](output/lianying-free-fixed-180s-pruned-suffix-revival.json)
- 循环伤害：`2,562,622,099.94`
- 循环 DPS：`14,236,789.44`
- 计入装备与附魔后的总 DPS：`14,716,535.15`
- 机制违规：`0`
...
新正式轴保持原有150个GCD主技能、雷表和橙武时点，只调整三组非GCD动作：首次雷内提前一拍下马以多积累1层龙驭，第六雷末段延后一拍下马以让1个龙牙额外消耗龙驭。循环伤害较上一正式轴提高`2,490,669.86`；后续`balanced`邻域复核无新增益，独立重放与机制审计均通过。六组主要伤害权重独立变化`±25%`时，新轴在整个连续误差盒内仍保持第一，最小领先`1,655,522.87`。
