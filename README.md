...
- 正式技能轴：[`output/lianying-free-fixed-180s-pair-anchor-wait.json`](output/lianying-free-fixed-180s-pair-anchor-wait.json)
- 循环伤害：`2,559,691,795.50`
- 循环 DPS：`14,220,509.98`
- 计入装备与附魔后的总 DPS：`14,700,255.68`
- 机制违规：`0`
...
240秒结果保留用于证明长时规划会主动让利短时前缀；当前主线已回到180秒。最新正式轴以两处共6帧等待同时调整任驰骋段和末次雷节奏，不减少主要技能，多结算1跳流血，并改善梅花枪法攻击相位；相对原无等待轴提高`733,672.18`循环伤害。固定等待末三雷重合成、首轮裁剪状态复活和无单点收益等待协同均未再增伤；GCD内真实事件断点仍未覆盖，因此不视为已经封盘。
