...
M5.14 已完成全部正式相邻雷块及末段的最佳优先`probe`。除开场块外，完整候选召回提高约1.7–5.1倍，但剩余块最佳新增轴损失均为`0.159%–0.471%`，没有新的候选进入0.1%修复门槛；此前107–128行的`0.0427%`候选修复仍为零收益。正式结果不变，下一步把同一算法用于已验证仅低`699,221.62`的79/106近优雷表，而不继续增加正式雷表预算。
